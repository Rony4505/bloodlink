...
import { AdminHealthcareCompaniesPanel } from "@/components/healthcare/AdminHealthcareCompaniesPanel";
...
type PanelTab = "facilities" | "companies";

function AdminHealthcareFacilitiesPanel() {
  const { t } = useLocale();
...
export function AdminHealthcarePanel() {
  const { t } = useLocale();
  const [tab, setTab] = useState<PanelTab>("companies");
    ...
    <section className="overflow-hidden rounded-2xl border border-[var(--line)] bg-white/90 shadow-sm">
      <div className="border-b border-[var(--line)] bg-[linear-gradient(160deg,#fff8f4,#ffffff)] px-5 py-4">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <button
            type="button"
            className={tab === "companies" ? "btn-primary" : "btn-ghost"}
            onClick={() => setTab("companies")}
          >
            {t.healthcareCompaniesTab}
          </button>
          <button
            type="button"
            className={tab === "facilities" ? "btn-primary" : "btn-ghost"}
            onClick={() => setTab("facilities")}
          >
            {t.healthcareFacilitiesTab}
          </button>
        </div>
      </div>

      {tab === "companies" ? <AdminHealthcareCompaniesPanel /> : <AdminHealthcareFacilitiesPanel />}
    </section>
  );
}
